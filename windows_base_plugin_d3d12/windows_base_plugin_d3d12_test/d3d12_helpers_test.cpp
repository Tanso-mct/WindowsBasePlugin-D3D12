...
TEST(DxHelpers, CreateDX12Factory)
{
    ...
    wbp_d3d12::CreateDX12Factory(factory);
...
TEST(DxHelpers, CreateDX12CommandQueue)
{
    ...
    wbp_d3d12::CreateDX12CommandQueue(device, commandQueue);
...
TEST(DxHelpers, CreateSwapChain)
{
...

    {
        ...
        wbp_d3d12::CreateSwapChain(factory, commandQueue, frameCount, frameIndex, hWnd, clientWidth, clientHeight, swapChain);
    }
...
TEST(DxHelpers, ResizeSwapChain)
{
    ...
    // Resize the swap chain
    {
        ...
        wbp_d3d12::ResizeSwapChain(frameCount, frameIndex, clientWidth, clientHeight, swapChain);
    }
...
TEST(DxHelpers, GetBuffersFromSwapChain)
{
    ...
    wbp_d3d12::GetBuffersFromSwapChain(swapChain, frameCount, buffers);
    ...
    for (int i = 0; i < frameCount; ++i)
    {
        ...
        wbp_d3d12::SetName(buffers[i], bufferName);
    }
...
TEST(DxHelpers, CreateCommandAllocator)
{
    ...
    wbp_d3d12::CreateCommandAllocator(device, commandAllocator);
...
TEST(DxHelpers, CreateViewport)
{
    ...
    wbp_d3d12::CreateViewport(viewport, clientWidth, clientHeight);
...
TEST(DxHelpers, CreateScissorRect)
{
    ...
    wbp_d3d12::CreateScissorRect(scissorRect, clientWidth, clientHeight);
...
TEST(DxHelpers, CreateBuffer)
{
    ...
    wbp_d3d12::CreateBuffer(device, buffer, size);
    EXPECT_NE(buffer.Get(), nullptr);

    // バッファの名前を設定
    std::wstring bufferName = L"TestBuffer";
    wbp_d3d12::SetName(buffer, bufferName);
}
...
TEST(DxHelpers, UpdateBuffer)
{
    ...
    wbp_d3d12::UpdateBuffer(buffer, initData.data(), size);
}
...
TEST(DxHelpers, CreateTexture2D)
{
    ...
    wbp_d3d12::CreateTexture2D(device, texture, width, height, DXGI_FORMAT_R8G8B8A8_UNORM, D3D12_RESOURCE_FLAG_NONE);
    EXPECT_NE(texture.Get(), nullptr);

    std::wstring textureName = L"TestTexture";
    wbp_d3d12::SetName(texture, textureName);
}
...
TEST(DxHelpers, CreateUploadHeap)
{
    ...
    wbp_d3d12::CreateUploadHeap(device, uploadHeap, size);
    EXPECT_NE(uploadHeap.Get(), nullptr);

    std::wstring uploadHeapName = L"TestUploadHeap";
    wbp_d3d12::SetName(uploadHeap, uploadHeapName);
}
...
TEST(DxHelpers, AddUploadTextureToCmdList)
{
    ...
    wbp_d3d12::CreateCommandList(device, commandAllocator, commandList);
    ...
    wbp_d3d12::CreateUploadHeap(device, uploadHeap, uploadHeapSize);
    EXPECT_NE(uploadHeap.Get(), nullptr);

    wbp_d3d12::ResetCommand(commandList, commandAllocator, nullptr);
...
TEST(DxHelpers, UploadTexture)
{
    ...
    UINT64 fenceValue = 0;
    wbp_d3d12::CreateFence(device, fence, fenceValue);
    ...
    wbp_d3d12::WaitForGPU(commandQueue, fence, fenceValue, fenceEvent);

    std::vector<ID3D12CommandList*> commandLists;
    commandLists.push_back(commandList.Get());
    wbp_d3d12::ExecuteCommand(commandQueue, commandLists.size(), commandLists);

    wbp_d3d12::WaitForGPU(commandQueue, fence, fenceValue, fenceEvent);

    wbp_d3d12::CloseFenceEvent(fenceEvent);
}